...
export function EnhancedStudentProgress() {
  ...
  const totalLessons = subjects.reduce((acc, s) => acc + s.lessonsCompleted, 0);
  const totalQuizzes = subjects.reduce((acc, s) => acc + s.quizzesCompleted, 0);
  const overallAvgScore = subjects.length > 0 ? Math.round(
